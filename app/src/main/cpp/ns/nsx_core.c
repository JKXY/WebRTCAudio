...
#include "../common_audio/real_fft.h"
#include "nsx_core.h"
#include "../common_audio/cpu_features_wrapper.h"
...
static void UpdateNoiseEstimate(NoiseSuppressionFixedC* inst, int offset) {
  ...
  tmp16 = WebRtcSpl_MaxValueW16C(inst->noiseEstLogQuantile + offset,
                                   inst->magnLen);
...
void WebRtcNsx_DataAnalysis(NoiseSuppressionFixedC* inst,
  ...
  maxWinData = WebRtcSpl_MaxAbsValueW16C(winData, inst->anaLen);
...
void WebRtcNsx_ProcessCore(NoiseSuppressionFixedC* inst,
  ...
  assert(0== inst->anaLen % 16);
  assert(0== inst->anaLen2 % 8);
  assert(inst->blockLen10ms> 0);
  assert(0== inst->blockLen10ms % 16);
